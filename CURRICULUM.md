...
| **30.** MCP | An MCP adapter, so MCP tools appear in the same `ToolRegistry` as native ones. Demonstrate with one reproducible server rather than restructuring the agent around MCP. | — |
